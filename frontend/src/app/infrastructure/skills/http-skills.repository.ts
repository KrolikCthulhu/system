...
} from '../../use-cases/skills/commands/skills.commands';
import { SkillsRepository } from '../../use-cases/skills/ports/skills-repository.port';
import {
	SkillCategoryDto,
	SkillDto,
	SkillLevelDto,
...
export class HttpSkillsRepository implements SkillsRepository {
	...
	updateSkill(command: UpdateSkillCommand): Observable<Skill> {
		return this.http
			.patch<SkillDto>(
				`${this.baseUrl}/admin/skills/${command.id}`,
				command,
				{ withCredentials: true }
			)
			.pipe(map(mapSkillDto), catchError(error => this.handleHttpError(error)));
	...
	updateCategory(command: UpdateSkillCategoryCommand): Observable<SkillCategory> {
		return this.http
			.patch<SkillCategoryDto>(
				`${this.baseUrl}/admin/skills/categories/${command.id}`,
				command,
				{ withCredentials: true }
			)
			.pipe(
	...
	updateLevel(command: UpdateSkillLevelCommand): Observable<SkillLevel> {
		return this.http
			.patch<SkillLevelDto>(
				`${this.baseUrl}/admin/skills/levels/${command.id}`,
				command,
				{ withCredentials: true }
			)
			.pipe(
